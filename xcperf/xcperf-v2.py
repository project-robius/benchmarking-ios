...
def main():
    ...
    game_performance_tables = [
        "displayed-surfaces-per-second",
        "device-thermal-state-intervals",
    ]
    metal_system_tables = ["core-animation-fps-estimate"]

    activity_monitor_trace_file = xcrun_record_template(
        device_id, process_id, activity_monitor_template
    )
    game_performance_trace_file = xcrun_record_template(
        device_id, process_id, game_performance_template
    )
    metal_system_trace_file = xcrun_record_template(
        device_id,
        process_id,
        metal_system_template,
        instrument="Core Animation FPS",
        windowed=False,
    )

    for table in activity_monitor_tables:
        exported_table_file = xcrun_export_trace_file_table(
            activity_monitor_trace_file, table
        )
        xml_to_csv(exported_table_file)

    for table in game_performance_tables:
        exported_table_file = xcrun_export_trace_file_table(
            game_performance_trace_file, table
        )
        xml_to_csv(exported_table_file)

    for table in metal_system_tables:
        exported_table_file = xcrun_export_trace_file_table(
            metal_system_trace_file, table
        )
        xml_to_csv(exported_table_file)


def xcrun_record_template(
    device_id, process_id, template_name, instrument="", windowed=True
):
...
def xcrun_export_trace_file_table(input_trace_file_name, table_name):
    output_file_name = (
        input_trace_file_name.replace(".trace", "") + "-" + table_name + ".xml"
    )
    ...
    options = [
        ...
        "--xpath",
        f'/trace-toc/run[@number="1"]/data/table[@schema="{table_name}"]',
        "--output",
        output_file_name,
    ]
